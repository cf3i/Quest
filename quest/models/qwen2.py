# Based on HuggingFace Qwen2 Model: models/qwen2/modeling_qwen2.py
# Adapted for Quest sparse attention

"""PyTorch Qwen2 model with Quest sparse attention."""
import math
...
class Qwen2QuestAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper, adapted for Qwen2 with Quest sparse attention"""
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # Qwen2 uses bias=True for q/k/v projections, bias=False for o_proj
    ...
    def _init_rope(self):
        # rope_theta is default to 1e4 for Qwen2, but can be configured
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                # For other scaling types, use default scale
                # Quest's rope kernel will handle the scaling
                self.rope_scale = 1.0
    ...
    def forward(
        ...
        assert bsz == 1, "Qwen2QuestAttention only supports batch size 1."
        assert hasattr(self, 'layer_idx'), "Qwen2QuestAttention requires layer_idx to inference."
        ...
        if q_len > 1:
            ...
            # Skipping layers is controled by PAGE_BUDGET, which is set in Qwen2Model.
        ...
        if not output_attentions:
            ...
            attn_weights = None  # Quest doesn't return attention weights
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen2QuestAttention(config=config, layer_idx=layer_idx)
