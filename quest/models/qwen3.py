# Based on Qwen3 Model: qwen3/modeling_qwen3.py
# Adapted for Quest sparse attention

"""PyTorch Qwen3 model with Quest sparse attention."""
import math
...
# Import Qwen3Config from local config file
from quest.models.qwen3_config import Qwen3Config
...
def debug_tensor(name, tensor, layer_idx):
    if tensor is None:
        print(f"[Layer {layer_idx}] {name}: None")
        return
    
    # 转换为 float32 打印统计信息，防止 overflow 显示不出来
    t_float = tensor.float()
    print(f"[Layer {layer_idx}] {name} | Shape: {tensor.shape} | Dtype: {tensor.dtype} | "
          f"Min: {t_float.min():.4f} | Max: {t_float.max():.4f} | Mean: {t_float.mean():.4f} | "
          f"NaNs: {torch.isnan(tensor).any().item()}")
    ...
    def _init_rope(self):
        ...
        if self.config.rope_scaling is None:
            ...
            if isinstance(scaling, dict):
                ...
                scaling_type = scaling.get("type", "default")
            else:
                factor = getattr(scaling, "factor", 1.0)
                scaling_type = getattr(scaling, "type", "default")
            self.rope_scale = factor if scaling_type == "linear" else 1.0
    ...
    def forward(
        ...
        if q_len > 1:
             ...
             if not iController.need_estimate():
                ...
                quest.utils.decode_topk(estimated_attn_score, iController)
                torch.cuda.nvtx.range_pop()
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3QuestAttention(config=config, layer_idx=layer_idx)
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        # Quest uses simplified attention mask for batch_size=1
        # embed positions
        if attention_mask is None:
            attention_mask = torch.ones(
                (batch_size, seq_length_with_past), dtype=torch.bool, device=inputs_embeds.device
            )
        # Align with llama: build causal mask (handles pad/eos equality warning)
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel):
    ...
    def quest_init(
        ...
        # Follow llama.py pattern: use num_attention_heads (query heads)
        # Quest's InferenceController handles GQA internally
        self.model.iController = InferenceController(
            num_layers=config.num_hidden_layers,
            num_heads=config.num_attention_heads,  # Use query heads (32), not KV heads (8)
