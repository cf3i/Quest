...
# Simplified layer_type_validation function (from transformers.configuration_utils)
def layer_type_validation(layer_types, num_hidden_layers):
    ...
    if layer_types is not None and len(layer_types) != num_hidden_layers:
        raise ValueError(
            f"The length of `layer_types` ({len(layer_types)}) must be equal to "
...
class Qwen3Config(PretrainedConfig):
    ...
    >>> from quest.models.qwen3 import Qwen3Model
    ...
    def __init__(
        ...
        rope_parameters: Optional[Union[dict, object]] = None,
